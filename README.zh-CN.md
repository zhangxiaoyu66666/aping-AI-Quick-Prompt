...
- **优化目标**：支持通用 LLM、文生图、ComfyUI / Stable Diffusion 字段适配、即梦 / Seedance 风格视频提示词、Veo 3、AI 编程、Skill 体系和自定义目标。
...
AI Quick Prompt 1.0.3 是当前公开 GPL 社区版本。当前代码库包含：
...
- GitHub 社区版不包含云同步入口；Microsoft Store 分支的云同步代码结构记录在 [docs/cloud-sync-architecture.md](docs/cloud-sync-architecture.md)。
...
即梦 / Dreamina / Seedance 相关的候选 Skill 研究清单见 [docs/skill-source-candidates.md](docs/skill-source-candidates.md)。当前内置的 `即梦导演 Skill` 和即梦模板是啊拼原创干净房间实现，不复制候选仓库的代码、提示词正文或 Skill 文件；候选仓库只有在许可证、导入范围、平台条款和隐私影响完成审查后，才会作为第三方内容进入内置模板或 Skill 包。
