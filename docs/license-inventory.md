...
Scope: current `Prompt Input Method` workspace, including .NET projects, Rust native OCR worker, patched OCR library, bundled OCR models, and copied native third-party source.
...
| `fire-eye-ocr-worker` | Local Rust crate | Project-local, not published | Optional local OCR worker | Migrated/adapted from Xiaxia Pet reference code. |
